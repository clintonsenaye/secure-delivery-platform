...
ci-config: ## Print the GitHub repository variables and policy values to set
	...
	@echo ""
	@echo "  $(BOLD)There are no repository SECRETS to set. Not one.$(RESET)"
	@echo ""
	@echo "  No AWS access key, because the pipeline proves its identity instead"
	@echo "  of presenting one. No signing key, because signing is keyless. No"
	@echo "  App key, because the deploy pull request is opened with the run's own"
	@echo "  GITHUB_TOKEN and a human closes and reopens it to start the gate."
	@echo "  That last one costs two clicks per deploy and it is why the"
	@echo "  credential inventory in docs/architecture.md section 27 still has no"
	@echo "  entry a human could copy."
	@echo ""
